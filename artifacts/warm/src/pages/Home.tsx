...
export default function Home() {
  ...
  // Reset pendingStop whenever the session ends (background kill, time-limit, etc.)
  useEffect(() => {
    if (!running) {
      setPendingStop(false);
      if (pendingStopTimer.current) { clearTimeout(pendingStopTimer.current); pendingStopTimer.current = null; }
    }
  }, [running]);
          ...
          {DURATION_OPTIONS.map((mins) => {
            ...
            return (
              <button
                ...
                disabled={running}
                className={`relative flex-1 min-h-[76px] flex flex-col items-center justify-center gap-1 py-3 px-2 rounded-2xl border transition-colors duration-300 disabled:cursor-not-allowed
                  ${locked
